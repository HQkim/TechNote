...
def create(request):
    # 5. create 경로로 요청이 들어옴(POST) [case1. 잘못된 입력]
    # 10. create 경로로 요청이 들어옴(POST) [case2. 올바른 입력]
    if request.method == 'POST':
        # 6. 11. 데이터가 입력된 종이를 가져옴 => ArticleForm으로 인스턴스화(빈종이 + 사용자 데이터)
        form = ArticleForm(data=request.POST)  # data= 는 생략 가능
        # 7. 12. 사용자가 입력한 데이터가 유효한지(올바른지) 확인
        if form.is_valid():
            # 13. 데이터를 db에 저장한다.
            form.save()
    ...
    # 8. [case1] 잘못된 데이터를 다시 입력 받기 위해서 context에 form을 담는다.
    context = {
        'form': form,
    }
...
def update(request, pk):
    ...
    context = {
        'article': article,     # article.pk로 redirect나 a태그에 사용하려고
        'form': form,
    }
